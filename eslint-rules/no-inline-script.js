...
export default {
  meta: {
    ...
    messages: {
      noInlineScript:
        "Inline script tags are not allowed. Move script content to external files.",
    },
  },
  ...
  create(context) {
    return {
      ...
      ScriptTag(node) {
        ...
        const hasContent =
          node.value && node.value.value && node.value.value.trim().length > 0;
        const hasSrc =
          node.attributes &&
          node.attributes.some((attr) => attr.key && attr.key.value === "src");

<script lang="ts">
	...
	import Status from '../prs-or-issues/Status.svelte';
...
<EventWrapperLite {event} name_first>
	{#if with_permission}set status to{:else}proposed status{/if}
	<Status type={issue_or_pr_table_item?.type ?? 'issue'} status={event.kind} xs={true} />
</EventWrapperLite>

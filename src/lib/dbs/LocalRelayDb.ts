import { addEvents, getEventsForFilters, IndexCache, openDB } from 'nostr-idb'
...
export async function getCacheEventsForFilters(
  filters: Filter[]
): Promise<NostrEvent[]> {
  const { cache_relay_db, cache_relay_index } = await getCacheRelayDb()
  const events = await getEventsForFilters(
    cache_relay_db,
    filters,
    cache_relay_index
  )
